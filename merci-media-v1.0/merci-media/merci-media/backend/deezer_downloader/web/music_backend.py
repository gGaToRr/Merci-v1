...
@sched.register_command()
def download_songs_from_list(song_titles: list, playlist_name: str, add_to_playlist: bool, create_zip: bool):
    """
    Downloads songs from a list of titles.
    Expected format: "Artist - Title" or just "Title"
    """
    songs_absolute_location = []
    for i, song_title in enumerate(song_titles):
        report_progress(i, len(song_titles))
        song_title_stripped = song_title.strip()
        if len(song_title_stripped) == 0:  # Skip empty lines
            continue
        try:
            # Search for the song on Deezer
            results = deezer_search(song_title_stripped, TYPE_TRACK)
            if len(results) == 0:
                print(f"Warning: Could not find '{song_title_stripped}' on Deezer")
                continue
            # Get the track ID from the first result
            track_id = results[0]['id']
            # Get song info and download
            song = get_song_infos_from_deezer_website(TYPE_TRACK, track_id)
            absolute_filename = download_song_and_get_absolute_filename(TYPE_PLAYLIST, song, playlist_name)
            songs_absolute_location.append(absolute_filename)
        except Exception as e:
            print(f"Warning: Could not download '{song_title_stripped}': {e}")
    
    update_mpd_db(songs_absolute_location, add_to_playlist)
    if len(songs_absolute_location) > 0:
        songs_with_m3u8_file = create_m3u8_file(songs_absolute_location)
        if create_zip:
            return [create_zip_file(songs_with_m3u8_file)]
        return make_song_paths_relative_to_mpd_root(songs_absolute_location)
    else:
        print("No songs were downloaded from the list")
        return []
